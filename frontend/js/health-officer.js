import {
  getCurrentUser,
  getHealthQueue,
  getHealthMessagesWithStudent,
  sendHealthMessageToStudent,
...
async function pollQueue() {
  try {
    const result = await getHealthQueue();
...
function renderMessages(items) {
  ...
  items.forEach((item) => {
    ...
    bubble.className = item.sender === 'health_officer' ? 'chat-bubble chat-bubble-mine' : 'chat-bubble chat-bubble-theirs';
    const text = document.createElement('span');
    text.textContent = item.text;
    bubble.appendChild(text);
    const meta = document.createElement('span');
    meta.className = 'chat-bubble-meta';
    meta.textContent = item.sender === 'health_officer' ? 'You' : 'Student';
...
async function pollThread() {
  ...
  try {
    const result = await getHealthMessagesWithStudent(selectedStudent.studentUserId);
    const items = Array.isArray(result?.items) ? result.items : [];
...
async function loadThread(student) {
  ...
  try {
    const [queueResult, messagesResult] = await Promise.all([
      getHealthQueue(),
      getHealthMessagesWithStudent(student.studentUserId),
    ]);
...
async function selectStudent(item) {
  ...
  renderQueue((await getHealthQueue().catch(() => ({ items: [] })))?.items ?? []);
...
document.getElementById('chat-form').addEventListener('submit', async (event) => {
  ...
  try {
    await sendHealthMessageToStudent(selectedStudent.studentUserId, selectedStudent.studentName, text);
    input.value = '';
    await pollThread();
    await pollQueue();
  } catch (error) {
...
async function start() {
  ...
  try {
    ...
    if (!['health_officer', 'admin'].includes(user?.role)) {
      queueMessage.textContent = 'This queue is restricted to Health Centre staff and administrators.';
      studentList.setAttribute('aria-busy', 'false');
      return;
    }
    startQueuePolling();
